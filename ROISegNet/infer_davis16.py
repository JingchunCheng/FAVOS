...
feat_dir  = 'featpool_davis16/'
det_dir   = '../siamese-fc-master/tracking/DAVIS2016/'


davis_dir = '../data/DAVIS2016/'
split_f   = '{}/ImageSets/480p/val.txt'.format(davis_dir)
file_out  = '../results/'
...
for idx in range(len(indices)):
    ...
    feat_name   = '{}/{}.mat'.format(feat_dir,clip)
    data        = io.loadmat(feat_name)
    feat_pool   = data['feats']


    det_name     = '{}/{}.mat.mat'.format(det_dir, clip)
    img_idx = np.int(ss)

    feat_name    = '{}/{}/{}'.format(file_out, clip, ss)
    ...
    if  dets.shape[0] > 0: 
        ...
        io.savemat('{}.mat'.format(feat_name), {'mask': outs2,'sim': sim, 'rois': input_rois})  
