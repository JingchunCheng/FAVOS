...
class Davis2016SiameseDataLayer(caffe.Layer):
...
#               self.label1 = imresize(self.label1,  size=(np.int(self.H*self.scale), np.int(self.W*self.scale)), interp="nearest", mode='L')
               self.img2   = imresize(self.img2,    size=(np.int(self.H*self.scale), np.int(self.W*self.scale)), interp="bilinear")
#               self.label2 = imresize(self.label2,  size=(np.int(self.H*self.scale), np.int(self.W*self.scale)), interp="nearest", mode='L')
            print >> sys.stderr, 'SCALE {}'.format(self.scale)


            if np.max(self.label1) == 0 or np.max(self.label2) == 0:
               continue
           
            if np.sum(self.get_bbox(self.label1)) == 0 or np.sum(self.get_bbox(self.label2)) == 0: 
            # if objects in either image are too small
               continue

            if self.fg_random:
                self.bbox  = self.get_rois(self.label1, self.box_num, 1)
            else:
                self.bbox  = self.get_bboxes(self.label1, self.box_num)
            self.rois  = self.get_rois(self.label2, self.box_num, self.fg_rate)
            self.label_sim = np.zeros([self.box_num, 1], dtype = np.uint8)

            for i in range(np.int(self.box_num*self.fg_rate)):
                self.label_sim[i] = 1
...
#            print >> sys.stderr,'label = {}'.format(self.label_sim)
            ...
            self.label1 = np.uint8(self.label1>0)
            self.label2 = np.uint8(self.label2>0)
            break
        ...
        print >> sys.stderr,self.label1.shape
        print >> sys.stderr,self.label2.shape
